...
def train_model(model, adam, train_data, dev_data, tokenizer, device, args):
    ...
    for epoch in range(args.num_epoch):
        ...
        print("Validation Acc: {}".format(eval_metric))
        if eval_metric > best:
            best = eval_metric
            if args.data_parallel and torch.cuda.is_available():
                torch.save(model.module, os.path.join(args.save_dir, 'styleExtractor_model'))
            else:
...
if __name__ == '__main__':
    ...
    config = MT5Config.from_pretrained(args.pretrain_model)
    model = StyleExtractor.from_pretrained(args.pretrain_model, config=config).to(device)
    if args.data_parallel and torch.cuda.is_available():
