...
def train_model(model, adam, train_data, dev_data, tokenizer, device, args):
    ...
    for epoch in range(args.num_epoch):
        ...
        for i, cur in enumerate(tqdm(train_data, desc='Epoch {}:'.format(epoch))):
            ...
            # s_loss = args.style_factor * s_loss_fct(style_repr, style_ref)
            ...
            loss = gen_loss + s_loss
            if i % 100 == 0:
                print("Iter {}:  Training Loss: {},  MLM Loss: {},  Style Loss: {}".format(i, loss.item(), gen_loss.item(), s_loss.item()))
            loss.backward()
            adam.step()
            adam.zero_grad()
