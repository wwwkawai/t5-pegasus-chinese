...
import numpy as np
import rouge
from model import StylePoem, StylePoemConfig
...
def default_collate(batch, length=None):
    """组batch
    ...
    if isinstance(elem, torch.Tensor):
        ...
        # for key in elem:
        #     for d in batch:
        #         if not isinstance(d[key], string_classes):
        ...
        if not all(len(elem) == elem_size for elem in it):
            batch = sequence_padding(batch, length=length)
...
def generate(test_data, model, tokenizer, args):
    ...
    with open(args.result_file, 'w', encoding='utf-8', newline='') as f:
        ...
        for feature in tqdm(test_data):
            ...
            style = feature['style']
            title = feature['title']
            content = {k: v for k, v in feature.items() if k not in ['raw_data', 'title', 'style']}
            ...
            writer.writerows(zip(style, gen, title))
            gens.extend(gen)
            if 'title' in feature:
...
if __name__ == '__main__':
    ...
    config = StylePoemConfig.from_pretrained(args.model, with_adain=args.with_adain)
    model = StylePoem.from_pretrained(args.model, config = config).to(device)
